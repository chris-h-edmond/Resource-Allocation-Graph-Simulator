...
class ResourceAllocationGraph:
    def __init__(self, root):
        ...
        # Process controls
        tk.Label(self.controls_frame, text="Processes (comma-separated):").grid(row=0, column=0, padx=5, pady=5)
        self.process_entry = tk.Entry(self.controls_frame, width=30)
        self.process_entry.grid(row=0, column=1, padx=5, pady=5)
        tk.Button(self.controls_frame, text="Add Processes", command=self.add_processes).grid(row=0, column=2, padx=5, pady=5)
        
        # Resource controls
        tk.Label(self.controls_frame, text="Resources (comma-separated):").grid(row=1, column=0, padx=5, pady=5)
        self.resource_entry = tk.Entry(self.controls_frame, width=30)
        self.resource_entry.grid(row=1, column=1, padx=5, pady=5)
        tk.Button(self.controls_frame, text="Add Resources", command=self.add_resources).grid(row=1, column=2, padx=5, pady=5)
    ...
    def add_processes(self):
        processes = self.process_entry.get().strip().split(',')
        if len(processes) > 3:
            messagebox.showerror("Error", "You can only add up to 3 processes at a time.")
            return
    ...
    def add_resources(self):
        resources = self.resource_entry.get().strip().split(',')
        if len(resources) > 3:
            messagebox.showerror("Error", "You can only add up to 3 resources at a time.")
            return
    ...
    def check_deadlock(self):
        try:
            ...
            if cycle:
                # Highlight the cycle
                for edge in cycle:
                    u, v = edge[:2]  # Unpack only the first two elements
